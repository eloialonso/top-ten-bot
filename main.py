...
def get_ai_suggestion(theme, number, previous_suggestions):
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Keep it brief (one short sentence). "
                "For example, if theme is 'Un animal pour convaincre un enfant de venir au zoo', "
                "a good answer for intensity 1 would be 'une huitre', "
                "a good intensity 5 would be 'un chat sauvage', "
    ...
    try:
        resp = openai.chat.completions.create(
            model="gpt-4o-mini", messages=messages, temperature=0.7, max_tokens=40
        )
...
def play_round(players, colors, round_number):
    ...
    order = [
        {"name": p["name"], "kind": p["kind"], "number": i}
    ...
    for idx, p in enumerate(order, 1):
        clr = colors[p["name"]]
        print(f"{idx}. {clr}{p['name']}{Style.RESET_ALL}")
    print()
